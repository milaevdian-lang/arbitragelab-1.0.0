...
class RegimeSwitchingArbitrageRule:
    ...
    def get_signal(self, data: Union[np.array, pd.Series, pd.DataFrame], switching_variance: bool = False,
        ...
        # Handle both DataFrame (old statsmodels) and numpy array (new statsmodels) formats
        smoothed_probs = res.smoothed_marginal_probabilities
        if isinstance(smoothed_probs, pd.DataFrame):
            last_probs = smoothed_probs.iloc[-1]
        else:
            last_probs = smoothed_probs[-1]
        
        current_regime = np.argmax(last_probs)
        smoothed_prob = last_probs[current_regime]
