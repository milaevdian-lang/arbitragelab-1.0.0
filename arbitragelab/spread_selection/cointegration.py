...
class CointegrationSpreadSelector(AbstractPairsSelector):
    ...
    def generate_spread_statistics(self, spread_series: pd.Series, log_info: bool = True) -> dict:
        ...
        spread_array = np.asarray(spread_series)
        statistics['hurst_exponent'] = get_hurst_exponent(spread_array)
        ...
        if log_info is True:
            # Handle both pandas Series and numpy array (numpy array doesn't have .name)
            series_name = spread_series.name if hasattr(spread_series, 'name') else 'unnamed'
            self.selection_logs.loc[series_name] = statistics
