...
class OptimalConvergence:
    ...
    def fit(self, prices: pd.DataFrame, mu_m: float, sigma_m: float, r: float, delta_t: float = 1 / 252):
        ...
        # Fit first regression
        X = sm.add_constant(x[:-1].reshape(-1, 1))
        lr_1 = sm.OLS(y_1, X).fit()

        self.lambda_1 = -lr_1.params[1]  # params[0] is intercept, params[1] is coefficient
        beta_1 = (lr_1.params[0] - self.r) / self.mu_m

        # Fit second regression
        lr_2 = sm.OLS(y_2, X).fit()
        self.lambda_2 = lr_2.params[1]  # params[0] is intercept, params[1] is coefficient
        beta_2 = (lr_2.params[0] - self.r) / self.mu_m
