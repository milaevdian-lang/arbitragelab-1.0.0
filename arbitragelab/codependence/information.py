...
def _mutual_info_from_contingency(contingency: np.array) -> float:
    """
    Calculate mutual information from a contingency matrix.
    
    MI(X,Y) = sum_{x,y} p(x,y) * log(p(x,y) / (p(x) * p(y)))
    
    :param contingency: (np.array) Contingency matrix (joint frequency counts).
    :return: (float) Mutual information value.
    """
    ...
    joint_prob = joint_prob + 1e-10
    marginal_x = marginal_x + 1e-10
    marginal_y = marginal_y + 1e-10
    
    # Calculate MI: sum(p(x,y) * log(p(x,y) / (p(x) * p(y))))
    mi = joint_prob * np.log(joint_prob / (marginal_x * marginal_y))
...
def get_mutual_info(x: np.array, y: np.array, n_bins: int = None, normalize: bool = False,
    ...
    if estimator == 'standard':
        ...
        mutual_info = _mutual_info_from_contingency(contingency)  # Mutual information
        ...
        mutual_info = _mutual_info_from_contingency(contingency)  # Mutual information using a copula
